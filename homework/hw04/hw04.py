...
def balanced(m):
    ...
    if total_weight(end(left(m))) * length(left(m)) == total_weight(
        end(right(m))
    ) * length(right(m)):
...
def replace_leaf(t, find_value, replace_value):
    ...
    def helper(t, find_value, replace_value):
        if is_leaf(t) and label(t) == find_value:
            return tree(replace_value)
        else:
            new_branches = [
                helper(child, find_value, replace_value) for child in branches(t)
            ]
            return tree(label(t), new_branches)

    return helper(t, find_value, replace_value)


def preorder(t):
    """Return a list of the entries in this tree in the order that theye
...
def div_interval(x, y):
    ...
    "*** YOUR CODE HERE ***"
    assert not (
        lower_bound(y) < 0 and 0 < upper_bound(y)
...
def quadratic(x, a, b, c):
    ...
    mid = -b / (2 * a)
    p1 = cal(mid)
    p2 = cal(lower_bound(x))
    p3 = cal(upper_bound(x))
    if mid < upper_bound(x) and mid > lower_bound(x):
        # print(p1, p2, p3)
        return interval(min(p1, p2, p3), max(p1, p2, p3))
    else:
        # print(p1, p2, p3)
        return interval(min(p2, p3), max(p2, p3))
...
def check_par():
    ...
    True
    """
    r1 = interval(5, 7)  # Replace this line!
    r2 = interval(5, 7)  # Replace this line!
    return r1, r2
