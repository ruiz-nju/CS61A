...
def missing_digits(n):
    ...
    if n == n % 10:
        ...
        a, b = (n // 10) % 10, n % 10
        return max(b - a - 1, 0) + missing_digits(n // 10)
...
def count_change(total):
    ...
    def helper(total, min):
        if total < min:
            ...
            return helper(total - min, min) + helper(total, min * 2)
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    if n >= 1:
        move_stack(n - 1, start, mid(start, end))
        print_move(start, end)
        move_stack(n - 1, mid(start, end), end)
