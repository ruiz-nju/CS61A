...
def make_withdraw(balance, password):
    ...
    def password_protected_withdraw(amount, psw):
        nonlocal wrong_password
        if len(wrong_password) == 3:
            return "Too many incorrect attempts. Attempts: " + str(wrong_password)
        elif psw == right_password:
            return bank("withdraw", amount)
        else:
            wrong_password.append(psw)
    ...
    while next_a is not None or next_b is not None:
        if next_a is None:
            ...
            if next_a < next_b:
                ...
                yield next_a
                next_a, next_b = next(iter_a, None), next(iter_b)
